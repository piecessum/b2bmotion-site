...
const BOILERPLATE_PATTERNS = [
  ...
  /\bавторизуйтесь\b/i,
  /\bреклама\b/i,
  // Cookie-баннеры и согласия (в т.ч. мн. число «cookies» и рус. «куки»)
  /cookies?/i,
  /\bкуки\b/i,
  // Юридическая «обвязка» страницы: политика ПДн, реквизиты оператора, копирайт
  /персональных данных/i,
  /в отношении обработки/i,
  /политик[а-яё]*\s+(?:конфиденциальн|в отношении|обработки|использования)/i,
  /оператор(?:ом)?\s+(?:по\s+)?обработк/i,
  /\bИНН\b/,
  /\bОГРН(?:ИП)?\b/,
  /\bКПП\b/,
...
function extractArticleText(html: string): string {
  let h = html
    ...
    .replace(/<!--[\s\S]*?-->/g, " ")
    // Служебные блоки страницы: футер (реквизиты + политика ПДн), сайдбары
    // («популярное»/«читайте также»), меню и формы (подписка, cookie-согласие).
    // Их <p> — главный источник мусора в пересказе, вырезаем целиком.
