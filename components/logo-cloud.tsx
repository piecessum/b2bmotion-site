          ...
          {[0, 1].map((set) => (
            <div key={set} className="flex items-center gap-6 px-3 shrink-0">
              {companies.map((company, i) => (
                <div
                  key={`${set}-${i}`}
                  className="flex-shrink-0 flex items-center justify-center h-24 px-8 rounded-xl bg-overlay-2 border border-glass-border hover:border-[#3B82F6]/20 hover:bg-overlay-4 transition-all duration-300 cursor-default"
                >
                  <Image
                    src={company.logo}
                    alt={company.name}
                    width={200}
                    height={200}
                    className="h-[80px] w-auto object-contain opacity-70 hover:opacity-100 transition-opacity dark:invert"
                  />
                </div>
              ))}
            </div>
