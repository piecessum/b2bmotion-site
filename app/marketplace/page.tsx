    ...
    <main ref={r} className="relative min-h-screen bg-page-alt noise-overlay">
      ...
      <section className="py-24 px-6">
        <div className="max-w-6xl mx-auto">
          <h2 className="reveal font-heading font-bold text-[clamp(32px,5vw,44px)] tracking-[-0.02em] text-heading text-center mb-16">Функционал маркетплейса</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {features.map((f, i) => (<div key={i} className="reveal p-6 bg-surface-hover rounded-2xl border border-border-default hover:border-[#3B82F6]/40 transition-all duration-500 glow-card"><div className="w-10 h-10 rounded-xl bg-[#3B82F6]/10 flex items-center justify-center mb-4"><f.icon className="w-5 h-5 text-[#3B82F6]" /></div><h3 className="font-heading font-semibold text-lg text-heading mb-2">{f.title}</h3><p className="text-sm text-body leading-relaxed">{f.desc}</p></div>))}
          </div>
        </div>
      </section>
      <section className="py-24 px-6 bg-page-alt">
        <div className="max-w-6xl mx-auto">
          <h2 className="reveal font-heading font-bold text-[clamp(32px,5vw,44px)] tracking-[-0.02em] text-heading text-center mb-16">Реализованные маркетплейсы</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {portfolio.map((p, i) => (<div key={i} className="reveal overflow-hidden rounded-2xl border border-border-default hover:border-[#3B82F6]/40 transition-all duration-500 glow-card"><div className={`h-48 bg-gradient-to-br ${p.gradient} flex items-center justify-center`}><span className="font-heading font-bold text-3xl text-heading/80">{p.name}</span></div><div className="p-6 bg-surface-hover"><p className="text-xs text-subtle uppercase tracking-wider mb-2">{p.type}</p><p className="text-sm text-body leading-relaxed mb-4">{p.desc}</p><a href={p.url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-sm font-medium gradient-text">Посмотреть <ArrowRight className="w-4 h-4" style={{color:'#3B82F6'}} /></a></div></div>))}
          </div>
        </div>
      </section>
      <section className="py-24 px-6">
        <div className="max-w-6xl mx-auto">
          <h2 className="reveal font-heading font-bold text-[clamp(32px,5vw,44px)] tracking-[-0.02em] text-heading text-center mb-16">Преимущества маркетплейса</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {benefits.map((b, i) => (<div key={i} className="reveal p-6 bg-surface-hover rounded-2xl border border-border-default hover:border-[#3B82F6]/30 transition-all"><div className="w-10 h-10 rounded-xl bg-[#8B5CF6]/10 flex items-center justify-center mb-4"><b.icon className="w-5 h-5 text-[#8B5CF6]" /></div><h3 className="font-heading font-semibold text-heading mb-2">{b.title}</h3><p className="text-sm text-body">{b.desc}</p></div>))}
          </div>
        </div>
      </section>
      <CTASection /><Footer />
