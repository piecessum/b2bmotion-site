...
const allClients = [
  ...
  { name: "База Электроматериалов", industry: "Электротехника", desc: "Федеральный дистрибьютор электротехнического оборудования в области энергетики и климатики", url: "https://electro58.ru/" },
    ...
    <main ref={r} className="relative min-h-screen bg-page-alt noise-overlay">
      ...
      <section className="relative pt-36 pb-20 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          <h1 className="reveal font-heading font-bold text-[clamp(36px,7vw,60px)] leading-[1.1] tracking-[-0.03em] mb-4"><span className="gradient-text">56+</span> <span className="text-heading">проектов</span></h1>
          <p className="reveal text-lg text-body">в 12+ отраслях по всей России и за рубежом</p>
        </div>
      </section>
      ...
      <section className="py-12 px-6">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((c, i) => (
              <div key={c.name} className="p-6 bg-surface-hover rounded-2xl border border-border-default hover:border-[#3B82F6]/40 transition-all duration-500 glow-card">
                <span className={`inline-flex px-3 py-1 rounded-full text-xs font-medium mb-3 ${tagColor[c.industry] || "bg-dimmest text-subtle"}`}>{c.industry}</span>
      ...
      <section className="py-24 px-6 bg-page-alt">
        <div className="max-w-6xl mx-auto">
          <h2 className="reveal font-heading font-bold text-[clamp(32px,5vw,44px)] tracking-[-0.02em] text-heading text-center mb-16">Отзывы наших клиентов</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {testimonials.map((t, i) => (
              <div key={i} className="reveal relative p-8 bg-surface-hover rounded-2xl border border-border-default overflow-hidden hover:border-[#3B82F6]/30 transition-all duration-500">
                ...
                <p className="text-lg text-subheading leading-relaxed mb-6 relative z-10">&quot;{t.quote}&quot;</p>
                <div className="flex items-center gap-3"><span className="font-heading font-semibold text-heading">{t.company}</span><span className="px-3 py-1 bg-dimmest rounded-full text-xs text-subtle">{t.industry}</span></div>
              </div>
            ))}
          </div>
        </div>
