...
interface WikiTechArticlePageProps {
  params: Promise<{ slug: string }>;
}

export function generateStaticParams() {
  return wikiTechArticles.map((article) => ({
    slug: article.slug,
  }));
}

export default async function WikiTechArticlePage({
  params,
}: WikiTechArticlePageProps) {
  const { slug } = await params;
  const article = wikiTechArticles.find((a) => a.slug === slug);

  if (!article) {
    notFound();
  }

  const relatedArticles = wikiTechArticles
    .filter(
      (a) =>
